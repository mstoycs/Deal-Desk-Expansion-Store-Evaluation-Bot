...
class ImageSimilarityAnalyzer:
    ...
    def compare_images(self, image1_url: str, image2_url: str) -> ImageSimilarityResult:
        ...
        weights = {"structural": 0.3, "feature": 0.3, "color": 0.25, "contour": 0.15}
    ...
    def _structural_similarity(self, img1: np.ndarray, img2: np.ndarray) -> float:
        ...
        try:
            ...
            logger.warning(
                "skimage not available, using fallback structural similarity"
            )
    ...
    def _fallback_structural_similarity(
...
class EnhancedBrandingAnalyzer:
    ...
    def analyze_branding(self, url: str, html_content: str) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "color_scheme": (
                    self._color_scheme_to_dict(color_scheme) if color_scheme else None
                ),
    ...
    def compare_branding(
        self, main_store_data: Dict, expansion_store_data: Dict
    ) -> Dict:
        ...
        try:
            ...
            for main_logo in main_logos:
                for expansion_logo in expansion_logos:
                    similarity = self.similarity_analyzer.compare_images(
                        main_logo["src"], expansion_logo["src"]
                    )
            ...
            return self._fallback_branding_comparison(
                main_store_data, expansion_store_data
            )
    ...
    def _calculate_analysis_confidence(
        self, logos: List[LogoInfo], color_scheme: Optional[ColorScheme]
    ) -> float:
    ...
    def _compare_color_schemes(
        self, scheme1: Optional[Dict], scheme2: Optional[Dict]
    ) -> float:
    ...
    def _detailed_color_comparison(
        self, scheme1: Optional[Dict], scheme2: Optional[Dict]
    ) -> Dict:
        ...
        return {
            ...
            "color_palette_match": self._compare_color_palettes(
                scheme1.get("color_palette", []), scheme2.get("color_palette", [])
            ),
        }

    def _compare_color_palettes(
        self, palette1: List[str], palette2: List[str]
    ) -> float:
